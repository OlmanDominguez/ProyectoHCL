namespace ProyectoHCL.Formularios.Bitacora
{
    partial class ShowBitacora
    {
        /// <summary>
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        /// Required method for Designer support - do not modify
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ShowBitacora));
            panel1 = new Panel();
            label7 = new Label();
            lb_accion = new Label();
            label6 = new Label();
            lb_fecha = new Label();
            label5 = new Label();
            lb_pantalla = new Label();
            label4 = new Label();
            lb_modulo = new Label();
            label1 = new Label();
            lb_user = new Label();
            label3 = new Label();
            lb_idBitacora = new Label();
            label2 = new Label();
            panel2 = new Panel();
            pictureBox5 = new PictureBox();
            btnCerrar = new Button();
            label11 = new Label();
            lb_descripcion = new Label();
            panel1.SuspendLayout();
            panel2.SuspendLayout();
            ((System.ComponentModel.ISupportInitialize)pictureBox5).BeginInit();
            SuspendLayout();
            // 
            // panel1
            // 
            panel1.BackColor = Color.LightGray;
            panel1.Controls.Add(lb_descripcion);
            panel1.Controls.Add(label7);
            panel1.Controls.Add(lb_accion);
            panel1.Controls.Add(label6);
            panel1.Controls.Add(lb_fecha);
            panel1.Controls.Add(label5);
            panel1.Controls.Add(lb_pantalla);
            panel1.Controls.Add(label4);
            panel1.Controls.Add(lb_modulo);
            panel1.Controls.Add(label1);
            panel1.Controls.Add(lb_user);
            panel1.Controls.Add(label3);
            panel1.Controls.Add(lb_idBitacora);
            panel1.Controls.Add(label2);
            panel1.Dock = DockStyle.Fill;
            panel1.Location = new Point(0, 73);
            panel1.Name = "panel1";
            panel1.Size = new Size(611, 335);
            panel1.TabIndex = 23;
            // 
            // label7
            // 
            label7.Anchor = AnchorStyles.Top;
            label7.AutoSize = true;
            label7.BackColor = Color.Transparent;
            label7.Font = new Font("Century Gothic", 12F, FontStyle.Bold, GraphicsUnit.Point);
            label7.ForeColor = Color.Black;
            label7.Location = new Point(60, 169);
            label7.Name = "label7";
            label7.Size = new Size(104, 19);
            label7.TabIndex = 44;
            label7.Text = "Descripción:";
            // 
            // lb_accion
            // 
            lb_accion.Anchor = AnchorStyles.Top;
            lb_accion.AutoSize = true;
            lb_accion.BackColor = Color.Transparent;
            lb_accion.Font = new Font("Century Gothic", 12F, FontStyle.Bold, GraphicsUnit.Point);
            lb_accion.ForeColor = Color.Black;
            lb_accion.Location = new Point(447, 129);
            lb_accion.Name = "lb_accion";
            lb_accion.Size = new Size(0, 19);
            lb_accion.TabIndex = 43;
            // 
            // label6
            // 
            label6.Anchor = AnchorStyles.Top;
            label6.AutoSize = true;
            label6.BackColor = Color.Transparent;
            label6.Font = new Font("Century Gothic", 12F, FontStyle.Bold, GraphicsUnit.Point);
            label6.ForeColor = Color.Black;
            label6.Location = new Point(379, 129);
            label6.Name = "label6";
            label6.Size = new Size(69, 19);
            label6.TabIndex = 42;
            label6.Text = "Acción:";
            // 
            // lb_fecha
            // 
            lb_fecha.Anchor = AnchorStyles.Top;
            lb_fecha.AutoSize = true;
            lb_fecha.BackColor = Color.Transparent;
            lb_fecha.Font = new Font("Century Gothic", 12F, FontStyle.Bold, GraphicsUnit.Point);
            lb_fecha.ForeColor = Color.Black;
            lb_fecha.Location = new Point(121, 129);
            lb_fecha.Name = "lb_fecha";
            lb_fecha.Size = new Size(0, 19);
            lb_fecha.TabIndex = 41;
            // 
            // label5
            // 
            label5.Anchor = AnchorStyles.Top;
            label5.AutoSize = true;
            label5.BackColor = Color.Transparent;
            label5.Font = new Font("Century Gothic", 12F, FontStyle.Bold, GraphicsUnit.Point);
            label5.ForeColor = Color.Black;
            label5.Location = new Point(60, 129);
            label5.Name = "label5";
            label5.Size = new Size(62, 19);
            label5.TabIndex = 40;
            label5.Text = "Fecha:";
            // 
            // lb_pantalla
            // 
            lb_pantalla.Anchor = AnchorStyles.Top;
            lb_pantalla.AutoSize = true;
            lb_pantalla.BackColor = Color.Transparent;
            lb_pantalla.Font = new Font("Century Gothic", 12F, FontStyle.Bold, GraphicsUnit.Point);
            lb_pantalla.ForeColor = Color.Black;
            lb_pantalla.Location = new Point(456, 84);
            lb_pantalla.Name = "lb_pantalla";
            lb_pantalla.Size = new Size(0, 19);
            lb_pantalla.TabIndex = 39;
            // 
            // label4
            // 
            label4.Anchor = AnchorStyles.Top;
            label4.AutoSize = true;
            label4.BackColor = Color.Transparent;
            label4.Font = new Font("Century Gothic", 12F, FontStyle.Bold, GraphicsUnit.Point);
            label4.ForeColor = Color.Black;
            label4.Location = new Point(379, 84);
            label4.Name = "label4";
            label4.Size = new Size(77, 19);
            label4.TabIndex = 38;
            label4.Text = "Pantalla:";
            // 
            // lb_modulo
            // 
            lb_modulo.Anchor = AnchorStyles.Top;
            lb_modulo.AutoSize = true;
            lb_modulo.BackColor = Color.Transparent;
            lb_modulo.Font = new Font("Century Gothic", 12F, FontStyle.Bold, GraphicsUnit.Point);
            lb_modulo.ForeColor = Color.Black;
            lb_modulo.Location = new Point(147, 84);
            lb_modulo.Name = "lb_modulo";
            lb_modulo.Size = new Size(0, 19);
            lb_modulo.TabIndex = 37;
            // 
            // label1
            // 
            label1.Anchor = AnchorStyles.Top;
            label1.AutoSize = true;
            label1.BackColor = Color.Transparent;
            label1.Font = new Font("Century Gothic", 12F, FontStyle.Bold, GraphicsUnit.Point);
            label1.ForeColor = Color.Black;
            label1.Location = new Point(60, 84);
            label1.Name = "label1";
            label1.Size = new Size(86, 19);
            label1.TabIndex = 36;
            label1.Text = "SubMenu:";
            // 
            // lb_user
            // 
            lb_user.Anchor = AnchorStyles.Top;
            lb_user.AutoSize = true;
            lb_user.BackColor = Color.Transparent;
            lb_user.Font = new Font("Century Gothic", 12F, FontStyle.Bold, GraphicsUnit.Point);
            lb_user.ForeColor = Color.Black;
            lb_user.Location = new Point(448, 39);
            lb_user.Name = "lb_user";
            lb_user.Size = new Size(0, 19);
            lb_user.TabIndex = 35;
            // 
            // label3
            // 
            label3.Anchor = AnchorStyles.Top;
            label3.AutoSize = true;
            label3.BackColor = Color.Transparent;
            label3.Font = new Font("Century Gothic", 12F, FontStyle.Bold, GraphicsUnit.Point);
            label3.ForeColor = Color.Black;
            label3.Location = new Point(379, 39);
            label3.Name = "label3";
            label3.Size = new Size(69, 19);
            label3.TabIndex = 34;
            label3.Text = "Usuario:";
            // 
            // lb_idBitacora
            // 
            lb_idBitacora.Anchor = AnchorStyles.Top;
            lb_idBitacora.AutoSize = true;
            lb_idBitacora.BackColor = Color.Transparent;
            lb_idBitacora.Font = new Font("Century Gothic", 12F, FontStyle.Bold, GraphicsUnit.Point);
            lb_idBitacora.ForeColor = Color.Black;
            lb_idBitacora.Location = new Point(156, 39);
            lb_idBitacora.Name = "lb_idBitacora";
            lb_idBitacora.Size = new Size(0, 19);
            lb_idBitacora.TabIndex = 33;
            // 
            // label2
            // 
            label2.Anchor = AnchorStyles.Top;
            label2.AutoSize = true;
            label2.BackColor = Color.Transparent;
            label2.Font = new Font("Century Gothic", 12F, FontStyle.Bold, GraphicsUnit.Point);
            label2.ForeColor = Color.Black;
            label2.Location = new Point(60, 39);
            label2.Name = "label2";
            label2.Size = new Size(96, 19);
            label2.TabIndex = 29;
            label2.Text = "ID Bitacora:";
            // 
            // panel2
            // 
            panel2.BackColor = Color.SteelBlue;
            panel2.Controls.Add(pictureBox5);
            panel2.Controls.Add(btnCerrar);
            panel2.Controls.Add(label11);
            panel2.Dock = DockStyle.Top;
            panel2.Location = new Point(0, 0);
            panel2.Name = "panel2";
            panel2.Size = new Size(611, 73);
            panel2.TabIndex = 22;
            panel2.MouseMove += panel2_MouseMove;
            // 
            // pictureBox5
            // 
            pictureBox5.BackColor = Color.Transparent;
            pictureBox5.Image = (Image)resources.GetObject("pictureBox5.Image");
            pictureBox5.Location = new Point(12, 12);
            pictureBox5.Name = "pictureBox5";
            pictureBox5.Size = new Size(50, 48);
            pictureBox5.SizeMode = PictureBoxSizeMode.StretchImage;
            pictureBox5.TabIndex = 22;
            pictureBox5.TabStop = false;
            // 
            // btnCerrar
            // 
            btnCerrar.BackColor = Color.Transparent;
            btnCerrar.BackgroundImage = (Image)resources.GetObject("btnCerrar.BackgroundImage");
            btnCerrar.BackgroundImageLayout = ImageLayout.Stretch;
            btnCerrar.FlatAppearance.BorderSize = 0;
            btnCerrar.FlatAppearance.MouseOverBackColor = Color.FromArgb(128, 128, 255);
            btnCerrar.FlatStyle = FlatStyle.Flat;
            btnCerrar.Location = new Point(585, 12);
            btnCerrar.Name = "btnCerrar";
            btnCerrar.Size = new Size(14, 17);
            btnCerrar.TabIndex = 31;
            btnCerrar.UseVisualStyleBackColor = false;
            btnCerrar.Click += btnCerrar_Click;
            // 
            // label11
            // 
            label11.Anchor = AnchorStyles.Top;
            label11.AutoSize = true;
            label11.BackColor = Color.Transparent;
            label11.Font = new Font("Segoe UI Semibold", 16F, FontStyle.Bold, GraphicsUnit.Point);
            label11.ForeColor = Color.White;
            label11.Location = new Point(200, 21);
            label11.Name = "label11";
            label11.Size = new Size(181, 30);
            label11.TabIndex = 28;
            label11.Text = "Registro Bitacora";
            // 
            // lb_descripcion
            // 
            lb_descripcion.Anchor = AnchorStyles.Top;
            lb_descripcion.AutoSize = true;
            lb_descripcion.BackColor = Color.Transparent;
            lb_descripcion.Font = new Font("Century Gothic", 12F, FontStyle.Bold, GraphicsUnit.Point);
            lb_descripcion.ForeColor = Color.Black;
            lb_descripcion.Location = new Point(163, 169);
            lb_descripcion.Name = "lb_descripcion";
            lb_descripcion.Size = new Size(0, 19);
            lb_descripcion.TabIndex = 45;
            // 
            // ShowBitacora
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            ClientSize = new Size(611, 408);
            Controls.Add(panel1);
            Controls.Add(panel2);
            FormBorderStyle = FormBorderStyle.None;
            Name = "ShowBitacora";
            Text = "ShowBitacora";
            Load += ShowBitacora_Load;
            panel1.ResumeLayout(false);
            panel1.PerformLayout();
            panel2.ResumeLayout(false);
            panel2.PerformLayout();
            ((System.ComponentModel.ISupportInitialize)pictureBox5).EndInit();
            ResumeLayout(false);
        }

        #endregion

        private Panel panel1;
        private Panel panel2;
        private PictureBox pictureBox5;
        private Button btnCerrar;
        private Label label11;
        public Label lb_idBitacora;
        public Label label2;
        public Label lb_user;
        public Label label3;
        public Label lb_pantalla;
        public Label label4;
        public Label lb_modulo;
        public Label label1;
        public Label label7;
        public Label lb_accion;
        public Label label6;
        public Label lb_fecha;
        public Label label5;
        public Label lb_descripcion;
    }
}